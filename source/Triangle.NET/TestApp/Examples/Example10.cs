...
namespace MeshExplorer.Examples
{
    public class Example10 : IExample
    {
        ...
        public Example10()
        {
            ...
            Description = "This example will show how to deal with degeneracies on a mesh boundary caused by nearly collinear points.";
        }
        ...
        public void Execute()
        {
            var iterationInfos = Run();
            foreach (var iterationInfo in iterationInfos)
            {
                InputGenerated(iterationInfo.Mesh, EventArgs.Empty);
                DarkMessageBox.Show($"{Name}" , $"{iterationInfo.Info}");
            }
        }

        public static IEnumerable<MeshInfo> Run()
        {
            ...
            var iterationInformation = string.Empty;
            for (int i = 0; i < 10; i++)
            {
                var mesh = poly.Triangulate();

                var list = CheckBoundary(mesh).ToList();
                iterationInformation = $"Iteration {i}: Everything is ok in mesh.";
                if (list.Count > 0)
                {
                    Console.WriteLine("Iteration {0}: found {1} invalid triangle(s).",
                        i, list.Count);
                    iterationInformation = $"Iteration {i}: found {list.Count} invalid triangle(s).";
                ...
                yield return new MeshInfo() { Mesh = (Mesh)mesh, Info = iterationInformation };
